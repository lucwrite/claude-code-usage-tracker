...
8. **`energy.py`** (opt-in via `--energy`) -- a rough energy-use estimate.
   ...
   The dashboard's Energy estimate section links to `methodology.html`, a
   full writeup of this same methodology and sourcing (generated by
   `generate_methodology()` in `web.py`, always written as a sibling of
   `report.html` so it travels with it to the hosted version too).
